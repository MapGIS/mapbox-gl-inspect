...
export function isInspectStyle(style) {
  return style.metadata && style.metadata['mapbox-gl-inspect:inspect'];
}

export function markInspectStyle(style) {
...
export function fixRasterSource(source) {
  if (
    ...
    return {
      ...
      tiles: source.tiles,
      mapgisOffset: source.mapgisOffset
    };
  }
  if (
    (source.type === 'raster' || source.type === 'raster-dem') &&
...
export function notifyVersion(mapboxgl) {
...
export function MapboxInspect(options) {
...
MapboxInspect.prototype.render = function () {
  if (this._showInspectMap) {
    if (this.options.useInspectStyle) {
      let style = fixStyle(markInspectStyle(this._inspectStyle()));
    ...
    if (this.options.useInspectStyle) {
      let style = fixStyle(this._originalStyle);
      this._map.setStyle(style);
    }
